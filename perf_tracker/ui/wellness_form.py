"""
ui/wellness_form.py
Formulaire de saisie wellness pour un joueur sélectionné.
...
INDICATORS = [
    ("sommeil",     "Sommeil",      "Qualité du sommeil"),
    ("humeur",      "Humeur",       "État d'esprit général"),
    ("energie",     "Énergie",      "Niveau d'énergie ressenti"),
    ("courbatures", "Courbatures",  "Douleurs musculaires"),
    ("stress",      "Stress",       "Niveau de stress"),
]


class WellnessForm(QWidget):
    """
    Formulaire wellness pour un joueur.
    Émet back_requested() pour retourner à la liste.
    """
    back_requested   = pyqtSignal()
    wellness_saved   = pyqtSignal()
    ...
    def _build_ui(self):
        ...
        title = QLabel(
            f"Wellness — {self.player['prenom']} {self.player['nom']}"
        )
        ...
        # ── Score total preview ──
        self.score_label = QLabel("Score total : —")
    ...
    def _update_score(self):
        ...
        filled = [v for v in self.values.values() if v is not None]
        if len(filled) == 5:
            score = round(sum(filled) / 5, 2)
            self.score_label.setText(f"Score total : {score} / 5")
            self.score_label.setStyleSheet(f"""
                color: {COLOR_GREEN};
                font-size: {FONT_SIZE_BODY}px;
                font-weight: 700;
            ...
            remaining = 5 - len(filled)
            self.score_label.setText(
                f"Score total : — ({remaining} indicateur(s) manquant(s))"
            )

    def _on_save(self):
        """Valide et enregistre le wellness en base."""
        ...
        try:
            create_wellness(
                ...
                sommeil     = self.values["sommeil"],
                humeur      = self.values["humeur"],
                energie     = self.values["energie"],
                courbatures = self.values["courbatures"],
                stress      = self.values["stress"],
            )
            self.save_btn.setEnabled(False)
            self.save_btn.setText("Enregistré !")
            self.wellness_saved.emit()
